...
# ---------------------------------------------------------------------------
# Portability layer (added for the Android/APK build).
# On Android the application directory is read-only and there is no `yt-dlp`
# console script or `ffmpeg` binary on PATH -- everything is a Python package
# or a bundled executable inside the APK. These env vars are set by main.py.
# ---------------------------------------------------------------------------
DOWNLOAD_DIR = os.environ.get(
    "RECLIP_DOWNLOAD_DIR", os.path.join(os.path.dirname(__file__), "downloads")
)
os.makedirs(DOWNLOAD_DIR, exist_ok=True)

if os.environ.get("RECLIP_YTDLP_MODULE") == "1":
    # Run yt-dlp as a module: `python -m yt_dlp`
...
def ytdlp_base():
    """Base yt-dlp command, including a bundled ffmpeg location if present."""
    base = list(YTDLP_CMD)
    if FFMPEG_LOC:
